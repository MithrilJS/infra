import {checkSecretExpiration, fail, reportRunError, run} from "../util.js"
...
export async function deployToGitHubPages(payload) {
    ...
    checkSecretExpiration("GITHUB_PAGES_TOKEN")

    const request = getRequest(process.env.INPUT_GITHUB_PAGES_TOKEN)
    ...
    async function cancelDeployment() {
        ...
        try {
            ...
            fail([
                "Canceling Pages deployment failed",
                error,
    ...
    try {
        try {
            ...
            if (artifactData.size_in_bytes > SIZE_LIMIT_BYTES) {
                ...
                fail(
                    `::warning::Uploaded artifact size of ${artifactData.size_in_bytes
            ...
            let message = `Failed to create deployment (status: ${e.status}) with build version ${payload.buildVersion}.`

            if (e.response.headers["x-github-request-id"]) {
                message += ` Request ID ${e.response.headers["x-github-request-id"]}`
            }

            if (e.status >= 500) {
                message += " Server error. Check https://githubstatus.com for a possible GitHub Pages outage and re-run the deployment at a later time."
            } else if (e.status === 400) {
                message += ` Responded with: ${e.message}`
            } else if (e.status === 403) {
                message += ' Ensure GITHUB_TOKEN has permission "pages: write".'
            } else if (e.status === 404) {
                message += ` Ensure GitHub Pages has been enabled: https://github.com/${payload.repo}/settings/pages`
            }
        ...
        for (;;) {
            ...
            if (errorCount >= MAX_ERROR_COUNT) {
                ...
                fail([
                    ...
                    `Failed with status code: ${errorStatus}`,
                ])
            }
