...
export async function getIDToken() {
    ...
    try {
        if (!process.env.ACTIONS_ID_TOKEN_REQUEST_URL) {
            fail("Unable to get `ACTIONS_ID_TOKEN_REQUEST_URL` env variable")
        }

        console.log(`::debug::ID token url is ${process.env.ACTIONS_ID_TOKEN_REQUEST_URL}`)

        if (!process.env.ACTIONS_ID_TOKEN_REQUEST_TOKEN) {
            fail("Unable to get ACTIONS_ID_TOKEN_REQUEST_TOKEN env variable")
        }

        const idTokenUrl = new URL(process.env.ACTIONS_ID_TOKEN_REQUEST_URL)
        ...
        fail([
            `Failed to get ID token (status code: ${statusCode})`,
            error,
