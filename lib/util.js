 ...
 * @param {Record<string, import("./config.js").Project>} projects
 */
export function getDeployToken(payload, projects) {
    const now = Date.now()

    if (!Object.hasOwn(projects, payload.packageName)) {
        fail(`Refusing to publish ${payload.packageName} as it is not allowlisted for`)
    }

    const project = projects[payload.packageName]

    if (project.location !== payload.repo) {
        fail(`Refusing to publish ${payload.packageName} as its repo is not allowlisted for`)
    }

    if (project.tokenExpiryDate <= now) {
        fail(`Refusing to publish ${payload.packageName} as its public token appears to have expired`)
    }

    if (localTokenExpiryDates[project.tokenName] <= now) {
        fail(`Refusing to publish ${payload.packageName} as the local deploy token for it (${project.tokenName}) appears to have expired`)
    }

    let authToken = process.env[`INPUT_${project.tokenName}`]

    if (!authToken || !(authToken = authToken.trim())) {
        fail(`Refusing to publish ${payload.packageName} as the local deploy token (${project.tokenName}) is empty or missing`)
    }
